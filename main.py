...
def run_slam(src_dir, log_dir, idx, split):
    ...
    start_t = slam.find_joint_t_idx_from_lidar(slam.lidar[0]['t']) # Finding the first t where there is a joint t
    ...
    # function
    slam.init_particles(n=1,p=xyth.reshape((3,1)),w=np.array([1]))
    slam.observation_step(t=start_t)

    # Initializing all the particles
    p = np.zeros((3,n), dtype=np.float64)
    slam.init_particles(n,p)
    slam.p[:,0] = deepcopy(slam.lidar[start_t]['xyth'])

    plot_ground_truth = np.zeros((3, len(slam.lidar) - start_t))
    plot_path = np.zeros_like(plot_ground_truth)
    i=0

    for t in tqdm.tqdm(range(start_t, len(slam.lidar))):
        slam.dynamics_step(t)
        slam.observation_step(t)
        plot_ground_truth[:,i] = deepcopy(slam.p[:, np.argmax(slam.w)].reshape((3,)))
        plot_path[:,i] = deepcopy(slam.lidar[t]['xyth'].reshape((3,)))
        i += 1

    plot_ground_truth_grid = slam.map.grid_cell_from_xy(plot_ground_truth[0], plot_ground_truth[1]) # Getting the xy coordinates
    plot_path_grid = slam.map.grid_cell_from_xy(plot_path[0], plot_path[1])

    plotmap = np.ones_like(slam.map.cells).astype(float)
    plotmap[slam.map.cells==1] = 0.0
    plotmap[slam.map.num_obs_per_cell == 0] = 0.5

    plt.figure()
    plt.imshow(plotmap, cmap='gray')
    plt.plot(plot_ground_truth_grid[1], plot_ground_truth_grid[0], 'g', label='Ground truth', linewidth = 0.7)
    plt.plot(plot_path_grid[1], plot_path_grid[0], 'b', label='Estimation', linewidth = 0.7)
    plt.legend()
    plt.savefig('data_3_1e-4.jpg')
    plt.pause(5)
